...
- [~] CSRF protection relies solely on `X-Mbeditor-Client: 1` header (`editors_controller.rb` line 11) — `skip_before_action :verify_authenticity_token` disables Rails CSRF entirely; a forged cross-origin request from an attacker-controlled page running in the same browser could include this header via a custom fetch; consider keeping CSRF token validation or at least verifying `Origin`/`Referer` against the host — **STILL OPEN: No Origin/Referer validation added; verify_mbeditor_client only checks custom header**
...
- [ ] `SAFE_BRANCH_NAME` duplicated in `EditorsController` (line 88) and `EditorChannel` (line 11) — both define identical `/\A[a-zA-Z0-9._\-\/]+\z/`; if one copy is updated the other silently diverges; extract to a shared constant in `Mbeditor` module or `ApplicationController`
- [ ] `RG_AVAILABLE` constant evaluated at class load time (`editors_controller.rb` line 17) — `system("which rg > /dev/null 2>&1")` runs once at boot; on systems where `rg` lives behind a PATH-managed version manager not active at boot the constant is permanently `false`, silently falling back to slower `grep`; use a lazy probe matching the rubocop/git availability pattern
- [ ] `build_tree` descends into symlinked directories without escape check (`editors_controller.rb` line 843) — `File.directory?(full)` returns `true` for symlinks pointing to directories; a symlink inside the workspace that points to a directory outside can expose its entire subtree in the file listing; add a `File.symlink?(full)` guard before recursing into a directory
- [ ] `GitService.resolve_path` has no symlink protection (`git_service.rb` lines 131-136) — uses `File.expand_path` only, not `File.realpath`; currently consumed by `GitDiffService#on_disk_content` for reading on-disk content; a crafted symlink that expands within the workspace but resolves outside passes the `start_with?` check; align with `ApplicationController#resolve_path` or add a clear doc comment marking it unsafe for user-supplied paths
...
- [ ] No `ActionCable::Channel::TestCase` coverage for `EditorChannel` — `save_state` and `save_branch_state` channel actions are exercised only via HTTP; add channel tests for: oversized payload (rejected without writing), invalid branch name (rejected), and successful state write round-trip
