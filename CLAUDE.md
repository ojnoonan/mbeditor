...
- All file paths must go through `resolve_path()` — `File.expand_path` collapses `..`, then containment is checked lexically against the workspace root, so no request can name anything outside it. Caps at 5 MB. **Symlinks are deliberately followed**: host apps link shared config dirs and sibling engines into their tree, and refusing those made the file open, then close itself a moment later. The sandbox governs what a request may *name*, not where a link a developer put in the checkout happens to point. `GitService.resolve_path` stays strict (`SafePath`) — git cannot operate outside its own repo anyway.
